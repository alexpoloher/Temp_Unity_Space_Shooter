...
public class PlayerShot : MonoBehaviour
{
    ...
    [Header("Sonidos")]
    [SerializeField] AudioClip sonidoChoqueConBala;
    [SerializeField] AudioClip sonidoChoqueConEscudo;
    ...
    private void OnTriggerEnter2D(Collider2D elOtro)
    {
        if (elOtro.gameObject.CompareTag("Enemy") || elOtro.gameObject.CompareTag("EnemyBullet"))
        {
            ...
            if (elOtro.gameObject.CompareTag("EnemyBullet"))
            {
                ...
                GestorSonido.Instance.EjecutarSonido(sonidoChoqueConBala);
            }
            Destroy(gameObject);
        } else if (elOtro.gameObject.CompareTag("Shield")) {

            GestorSonido.Instance.EjecutarSonido(sonidoChoqueConEscudo);
            Destroy(gameObject);
        }
