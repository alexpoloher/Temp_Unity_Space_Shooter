...
public class CanvasManager : MonoBehaviour
{
    ...
    [SerializeField] TextMeshProUGUI textoPuntuacionEnMuerte;
    ...
    void Update()
    {
        ...
        if (GestorPlayer.Instance.nivelActual == GestorPlayer.Instance.nivelMaximo)
        {
            ...
            barraExp.fillAmount = GestorPlayer.Instance.puntuacionActual / GestorPlayer.Instance.puntuacionParaSiguienteNivel;
        }
    ...
    public void MostrarPantallaMuerte(float puntuacionFinal) {

        Color opacidad = textoMuerte.color;
        opacidad.a = 0;
        textoMuerte.color = opacidad;       //Se pone a 0 la opacidad
        fondoMuerte.gameObject.SetActive(true);
        StartCoroutine(MostrarMensajeMuerte(puntuacionFinal));

    
    }

    IEnumerator MostrarMensajeMuerte(float puntuacionFinal) {
        ...
        textoPuntuacionEnMuerte.gameObject.SetActive(true);
        textoPuntuacionEnMuerte.text = "Puntuación: " + puntuacionFinal;
