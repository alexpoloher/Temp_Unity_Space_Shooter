...
public class GestorPlayer : MonoBehaviour
{
    ...
    [SerializeField] AudioClip sonidoAlarma;
    private float puntosApareceBoss = 8000;
    ...
    private bool haSpawneadoBoss = false;
    ...
    void onSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        {
            ReiniciarParametros();
            GestorSonido.Instance.IniciarMusicaNivel(1);
        } else if (scene.name.Equals("SceneFase2")) {
            GestorSonido.Instance.IniciarMusicaNivel(2);
        } else if (scene.name.Equals("SceneFase3")) {
            canvasManager.ActualizarPuntosTotales(puntuacionTotal);
            GestorSonido.Instance.IniciarMusicaNivel(2);
        }

        //Se reinician ciertos parámetros 
        //De PowerUps
        tienePowerUp = false;
        tiempoRestantePowerUp = duracionMaximaPowerUp;
    ...
    IEnumerator MostrarVictoria() {
        ...
        GestorVictoria gestorVictoria = FindAnyObjectByType<GestorVictoria>();
    ...
    IEnumerator Morir() {
        ...
        canvasManager.MostrarPantallaMuerte(puntuacionTotal);
    ...
    public void AplicarUpgrade(UpgradeState upgrade) {
        ...
        switch (upgrade.upgrade.TipoUpgrade) {
            ...
            case UpgradeObject.enumTipoUpgrade.Vida:
                ...
                vidaActual += upgrade.upgrade.CantidadEfecto;
    ...
    public void SumarExp(float expRecibida)
    {
        ...
        if (puntuacionTotal >= puntosApareceBoss && SceneManager.GetActiveScene().name.Equals("SceneFase3") && !haSpawneadoBoss) {
            haSpawneadoBoss = true;
            GameObject[] listaEnemigosEnEscena = GameObject.FindGameObjectsWithTag("Enemy");
            foreach (GameObject gameObject in listaEnemigosEnEscena) { 
                Destroy(gameObject);
            }

            EnemySpawner enemySpawner = GameObject.FindAnyObjectByType<EnemySpawner>();
            enemySpawner.DejarDeSpawnearPorBoss();
            StartCoroutine(SpawnearBoss());
    ...
    IEnumerator SpawnearBoss() {
        GestorSonido.Instance.EjecutarSonido(sonidoAlarma);
        yield return new WaitForSeconds(3f);
        BossSpawner bossSpawner = GameObject.FindAnyObjectByType<BossSpawner>();
        bossSpawner.SpawnearBoss();
    }
    void SubirNivel()
    {
